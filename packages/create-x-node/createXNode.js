...
function init() {
    const program = new commander.Command(packageJson.name)
        ...
        .option(
            ...
            "use a non-standard version of x-scripts"
        )
        .allowUnknownOption()
        .on("--help", () => {
            console.log(
    ...
    if (typeof projectName === "undefined") {
        ...
        console.log(
            `  ${chalk.cyan(program.name())} ${chalk.green("my-x-node")}`
        );
        console.log();
        console.log(
    ...
    Promise.all([
        ...
        Promise.all([
            ...
            .then(async ({ packageInfo, supportsTemplates, templateInfo }) => {
                ...
                const templateName = supportsTemplates
                    ? templateInfo.name
                    : undefined;
            ...
            .catch(reason => {
                ...
                if (!remainingFiles.length) {
                    ...
                    console.log(
                        `Deleting ${chalk.cyan(
                            `${appName}/`
                        )} from ${chalk.cyan(path.resolve(root, ".."))}`
                    );
...
function getInstallPackage(version, originalDirectory) {
    let packageToInstall = "x-scripts";
...
function getTemplateInstallPackage(template, originalDirectory) {
    let templateToInstall = "cxn-template";
    if (template) {
...
function checkAppName(appName) {
    ...
    const dependencies = ["x-scripts"].sort();
